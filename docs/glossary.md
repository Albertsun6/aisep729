...
> 依据 `docs/research/AI时代评审门禁-调研.md` v2（五路调研 + 异构终审 Dissent 13 条修订）

| 名词 | 定义 |
|---|---|
| **三通道契约** | 阻断权的无歧义划分：①**确定性阻断**（探针/lint/测试/SAST，唯一 blocker 来源，LLM 不得单独解除）②**LLM 分诊**（对①出 `likely-FP` 建议，解除须人审）③**LLM 自主发现**（默认 advisory，经 shadow 验证达标才可升阻断） |
| finding source | 每条 finding 必须标明来自哪个通道：`deterministic` / `llm-triage` / `llm-advisory` |
| 闭环状态机 | `open → fixed / false-positive / accepted-risk → reopened`；fixed 绑验证证据，后两态须独立人类署名+理由（accepted-risk 另需到期日与跟踪票据）；**安全 critical 禁止 accepted-risk** |
| 覆盖声明 | 评审必须输出「已审/未审文件清单」——**未审 ≠ 没问题**，超时/截断/解析失败不得输出为零 finding（fail-closed） |
| 抑制清单 | agent prompt 里明确写"不要报什么"（风格偏好/理论风险/纵深建议/已被上游覆盖）——实践证明这是最有价值的 prompt 工程 |
| 注意力锚定 | AI 摘要给出"重点看这几处"后人只看这几处的现象——故高风险路径须**先人审再看 AI 摘要** |
| 橡皮图章 | 有人点了 approve 但未真正评审；门禁最常见失效（无评审直接合并 PR +31.3%）→ 判据要测评审行为不测审批事件 |
| break-glass 率 | 绕过门禁的使用率；**只作"例外使用率"，不可当信任或 precision 指标**（低可能是信任，也可能是开发者直接修掉误报或组织压力） |
| approver≠author | 授权归因硬规则：批准者须为人类且非作者/最后 push 者，以**服务端 review 事件**为准（provenance trailer 只作披露，可伪造） |
| 规模升档 | >400 行或 >15 文件升档、>1000 行默认拆分；计数排除生成/lock/vendored 文件；标为**默认启发值非实证安全线** |
| 三级实施包 | 起步级（lint+test+secret scan 阻断，AI 全 advisory）/ 成长级（加 SAST + 结构化闭环 + 抽样标注）/ 成熟级（才启用风险模型与经验证的 AI 阻断权） |

## 阶段 5（发布与运营）

| 名词 | 定义 |
|---|---|
| PRR | 生产就绪评审（Google SRE）：上线前逐项核对的清单，每项须带**证据 + 人类核对人**（宪法 C14） |
| LCE | 发布协调工程：把 launch 做成显式门禁而非"合并即上线" |
| **部署 ≠ 发布** | 代码部署到生产 ≠ 对用户可见；用 feature flag（**默认关**）解耦，使回滚不必回滚代码 |
| 金丝雀步长 | 灰度放量的阶梯（如 1%→10%→50%→100%），每阶有**中止判据** |
| 中止判据 | 触发即停止放量并回滚的可观测条件（错误率/延迟/业务指标阈值） |
| 回滚 RTO | 从决定回滚到恢复正常的目标时间；**未演练的回滚预案=纸面预案**，不计为已就绪 |
| 不可逆点 | 数据库迁移等无法回退的操作；用 expand/contract 模式把不可逆点推后并缩小 |
| SLI/SLO/错误预算 | 服务水平指标/目标/预算；错误预算耗尽即触发发布冻结（错误预算策略） |
| 告警接线 | 每条告警必须指向 runbook 的具体小节——**没有 runbook 的告警是噪音** |
| runbook 三节 | 启动 / 回滚 / 故障处置，三节均须实质非空（SPEC-22） |
| 观察窗 | 发布后的重点观察期，期内不叠加新变更 |
| 无责 postmortem | 事故复盘对事不对人，产出改进项回流阶段0/1 |
| 门禁③双路径校验 | 阶段5 入口不读文件读事实：远程走 `gh pr view`（PR=MERGED 且 checks 绿），无 gh 时降级为「merge commit + 全量测试绿」；**两条都证不出即 fail-closed 拒绝** |

## 阶段 6（退役）

| 名词 | 定义 |
|---|---|
| advisory deprecation | 建议性退役：标记弃用但仍可用，靠沟通推动迁移（Google SWE book 两型之一） |
| compulsory deprecation | 强制退役：有明确截止日且**必须提供迁移工具**，到期停服 |
| 迁移工具 | codemod/脚本/兼容层——强制退役必备，"请大家自己改"不算迁移方案 |
| deprecation owner 与资金 | 退役是有成本的工程项目，须有负责人与预算；无人负责的退役=永远退不掉 |
| 依赖方盘点 / 调用量证据 | 谁在用、用多少——**凭证据不凭印象**决定能否下线 |
| 数据处置三态 | 迁移 / 归档 / 销毁，每份数据必须落其一并有对账校验 |
| 可逆窗口 | 下线后仍可恢复的时间段，过窗即不可逆 |
| 提前期 | 通知到停服的最短间隔（合规与合同可能有硬性要求） |
| Sunset 头 | RFC 8594：HTTP `Sunset` 响应头，机器可读的下线预告 |
| 支持期义务 | [EU CRA](https://eur-lex.europa.eu/eli/reg/2024/2847/oj/eng) 默认 ≥5 年漏洞处理义务——**退役不是免责，是有法定尾巴的** |
| 僵尸服务 | 已宣布退役却仍在跑、仍在计费、仍无人维护的服务——退役检查表要能证明它真的死了 |
| 成本归零 | 退役完成的可验证判据之一：账单归零 + 密钥吊销 |
| retired 指路 | 归档后留下指向替代方案的说明，而非 404 |
