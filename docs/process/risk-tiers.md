...
|---|---|---|---|---|
| **高** | **规则定义**：`docs/constitution.md`<br/>`docs/process/risk-tiers.md`<br/>`docs/process/skills-manifest.md`<br/>`docs/process/stages/**`<br/>`docs/architecture/adr/**`<br/>**门禁执行体**：`scripts/check-*.sh`<br/>`.claude/skills/*/scripts/check-*.sh`<br/>`scripts/ratchet.sh`<br/>`ops/**`<br/>`scripts/lib/**`<br/>`tests/probe-negative/**`<br/>`bin/**`<br/>`quality-baseline.txt`<br/>**评审能力本身**：`.claude/agents/**`<br/>`.claude/skills/e2e-review/**`<br/>`.claude/skills/*/SKILL.md`<br/>`.claude/skills/*/templates/**`<br/>**门禁台账**：`specs/*/{prfaq,prd,spec,review,release,deprecation}.md`<br/>**分发信任根**：`.claude-plugin/**`<br/>`.gitignore`<br/>**权限与执行边界**：`.github/workflows/**`<br/>`.github/actions/**`<br/>`.github/CODEOWNERS`<br/>`.claude/settings*.json`<br/>`.claude/hooks/**` | 改的是**规则本身或门禁本身**——错了会让后续所有检查失效（"改规则绕规则"是最危险的路径）。<br/><br/>**三条实测教训**（异构评审 #6 · reviewer #8 · security #4 各自独立指出）：<br/>① `.claude/agents/reviewer.md` 若只算"`*.md` 文档" → 低档 → 把它改成"永远无 finding"即可**关掉整套内环，且无人审**<br/>② `scripts/check-*.sh` 是四个 required check 的**执行体**，若只算中档，改成 `exit 0` 后 required check 变成真空绿<br/>③ `bin/e2e` 一次改动会污染**所有新客户仓**（漏发 risk-tiers 的事故正是这么发生的）<br/>④ **七个阶段探针实际住在 `.claude/skills/*/scripts/check-*.sh`** —— 它们才是六道门禁的真正执行体，却因落在 `.claude/skills/**` 只算中档（自举评审定档时发现）。**实测把 `check-prfaq.sh` 改成 `exit 0`：一个空 prfaq 当场通过。**<br/>⑤ **门禁台账本体曾被定为最低档、人审「否」**（安全评审 critical #4）：`- 批准人：` / `- 决定：` 两行就是全部批准证据，改「打回」→「批准」即直接改判。按本表自己的判据，答案显然是「会失效」。<br/>⑥ **模板与 SKILL.md 曾只算中档**：模板是门禁块的**出生形态**——把 `- 决定：<待填>` 改成 `- 决定：go`，此后**每份新制品天生已批准**；SKILL.md 里写着「第 0 步跑 `--gate-only`」，删掉该节 C1 的本地执行力归零。<br/>⑦ **`.gitignore` 曾是低档**：它是 `.claude/settings.local.json`（权限放宽覆盖文件）不入库的**唯一屏障**，删掉那一行，放宽后的 allow 列表即可提交进公开仓。<br/>　　⚠️ **但暴露面比初判小**：同一实测显示该改动**被负样本套件抓住**（负样本期望探针拒绝坏样本），且七个阶段探针**全部有负样本覆盖**、而 `tests/probe-negative/**` 本身是高档。故仅改探针不改样本 → 被拦；两者同改 → 已落入高档。补入高档属**纵深防御**，针对的是「将来新增探针却忘了配负样本」这条真实路径 | reviewer + security 双 agent + **异构评审**（跨模型，宪法 C12）+ 辩论矩阵留痕 | **强制**，且 approver≠author（C14）<br/>⚠️ 当前试点仓**未启用**，见 §执行层「当前状态」 |
| **中** | `.claude/skills/**`（除 e2e-review 与各 skill 的 `scripts/check-*.sh`）<br/>`scripts/**`（除 `check-*.sh` 与 `lib/`）<br/>`tests/**`（除 `probe-negative/`）<br/>`src/**` `lib/**` `app/**`<br/>业务代码（含单文件产品如 `index.html`） | 可执行代码，错了有真实后果，但有测试与探针兜底 | reviewer 单 agent + 可执行验证必过 | 建议（CI 绿可自动合并，除非 finding 有 block） |
| **低** | `docs/**`（除上列高风险项）<br/>`specs/**` 中**非门禁台账**的文件（如 `plan.md` `tasks.md` `stories.md`）<br/>`*.md` 文档类<br/>`README*` | 文字与制品记录，错了改起来便宜，且探针已覆盖结构 | 仅 CI 探针（结构/链接/拼写） | 否 |
