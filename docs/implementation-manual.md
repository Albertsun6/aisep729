...
而在于**绝大多数主张都有一个会失败的可执行断言钉着**——缺乏强制力的边界见 §7，那里逐条写明了哪些主张目前只有建议力。

在自建 + 自用的过程中，这套门禁抓到了 **30+ 条真实缺陷**，其中相当一部分
**光看代码、光靠"我觉得对了"极难在早期发现**（这些坑最初也是人踩出来的，只是踩到时往往已经晚了）。下面这张表是本手册的核心：
...
**处置**：`scripts/check-skill-deps.sh` 守住**显式路径引用**这一部分（能力边界见下）。
...
**设计要点**（否则会漏报）：
- 违规身份 = `工具:规则:文件:内容指纹`，**不含行号** —— 实测中未出现行号漂移导致的误报
- 判定用**集合差**（`comm -13`），不是数量比较 ——
  数量比较有洞：删一个旧违规 + 加一个新违规，总数不变但质量变了
- `quality-baseline.txt` 属**高风险路径**，按规范改基线须人审（防"改基线绕棘轮"）。
  ⚠️ **但这是规范不是强制**：分档目前依赖 `review.md` 自填，手填成低档探针发现不了（§7.1）

### 4.4 绞杀（逐块替换，不做大爆炸重写）

先在热点清单的 Top-N 里选**改动频繁 × 体量适中**的模块，按绿地路径走完整六门禁，
其余部分**在遵守分档规范的前提下**由棘轮控制不恶化。**不设"全量重构"里程碑。**
...
制品尾部的四行门禁块（批准人 / 决定 / 日期 / 备注）是**过程留痕，不是防篡改的批准凭证**。
任何有写权限的人都能把 `打回` 改成 `批准`，git 会如实记录是谁改的，但**改动本身不会被拦住**。

安全评审在这里抓到过一个 **critical**，值得完整讲，因为它说明"看起来有门禁"能错到什么程度：

> 台账解析曾用「全文首个 `- 决定：` 行」。执行者只要在正文**任意早处**写一行 `- 决定：go`，
> 尾部的真实门禁块保持 `<待填>`，探针就判 PASS、下游串锁打印 `GATE0: go ✓`。
> 而**人翻到文末只看到「待批」** —— 批准在人的视野里被隐形伪造了。
> 已实测复现，并已修复为「尾部锚定 + 全文出现 >1 条决定行一律判 UNKNOWN（歧义即拒绝）」。

**所以请这样理解两种模式**（ADR-009）：

| | 文本台账（试点模式） | 服务端事件（企业模式） |
|---|---|---|
| 能证明什么 | **有人在某个时刻写下了一个决定** | **某个身份在服务端批准了某个 commit** |
| 防不防篡改 | ❌ 不防 —— 有写权限即可改 | ✅ 防 —— 审批事件由平台记录，PR 作者改不了 |
| 能否声称"有门禁" | **不能** —— 只能说"有过程记录" | 可以，但须附行为证明 |

**升档到高档只是缓解**（改台账现在需要高档评审），**不是解决**。
...
## 7.4 许可（用之前先看清用的是哪一部分）

本仓**双许可**，详见仓根 [`LICENSE`](../LICENSE)：

| 部分 | 许可 | 你需要做什么 |
|---|---|---|
| **代码**（`.claude/` `scripts/` `ops/` `bin/` `tests/` `.github/`） | **Apache-2.0** | 保留声明、**标注修改**、遵守专利条款。可闭源再分发 |
| **文档与方法论**（`docs/` `specs/` `USAGE.md` 及调研报告） | **CC BY-SA 4.0** | **署名** + **相同方式共享**：基于本方法论做的培训材料/课程/衍生手册，必须同样以 BY-SA 发布，**不得转为闭源** |

**企业内部使用完全不受限**（这是有意的：没选"非商业"，因为那会挡住企业内用，
而企业内用正是本项目最重要的传播路径）。
...
| 验证基线 | 负样本 **142/142**（`check-readme.sh` 与实跑对账，改错必红）；六门禁全链路 PASS；demo 仓 CI 四 job 全绿 |
