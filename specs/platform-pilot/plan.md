...
|---|---|---|---|---|
| claude-code-action 在私有仓的配置/计费/权限未实测 | **S1：建最小测试私有仓跑 @claude workflow**（提前至首日——prfaq 深坑原判；仅作 CI 冒烟，主开发仍本地，需用户确认见门禁②） | 半天 | **M1 首日** | 外环降级为纯质量门禁（lint/test），AI 评审留内环——记 ADR |
